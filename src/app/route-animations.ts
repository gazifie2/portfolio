...
function slideTo(direction: any) {
  ...
  return [
    ...
    query(':enter', [
      style({ [direction]: '-100%', opacity: 1, height: '100%' })
    ]),
    group([
      query(':leave', [
        //animate('600ms ease-in-out', style({ opacity:0} )),
        animate('600ms ease-in-out', style({ [direction]: '100%', opacity: 1 }))
      ], optional),
      query(':enter', [
...
function zoomIn(originX?: any, originY?: any) {
  ...
  return [
    ...
    query(':enter', [
      style({ 'transform-origin': originX + ' ' + originY, transform: 'scale(0.5)', opacity: 0 })
    ]),
    group([
      query(':leave', [
        //animate('600ms ease-in-out', style({ opacity:0} )),
